...
class UsdtToCzusdToCl8yToAssetwalletWorkflowCalculator(ArgumentCalculator):
    """Calculate router arguments for the USDT→CZUSD→CL8Y→AssetWallet workflow."""
    ...
    def calculate_args(self, input_data: Optional[Dict[str, Any]] = None) -> CalculatorResult:
        ...
        the output to the AssetWallet address.
        ...
        cl8y_token_address = _to_checksum(web3, input_data.get("cl8y_token_address"))
        ...
        min_swap_amount = _to_decimal(input_data.get("min_swap_amount", "25"))
        max_swap_amount = _to_decimal(input_data.get("max_swap_amount", "75"))
        slippage_percent = _to_decimal(input_data.get("max_slippage_percent", "2"))
        precision = int(input_data.get("random_precision", 4))
        ...
        router_abi = load_contract_abi(self.ROUTER_ABI_PATH)
        ...
        path = [usdt_token_address, czusd_token_address, cl8y_token_address]

        min_amount_out = _apply_slippage(
...
def _apply_slippage(
    *,
    ...
    """Quote router output for the path and apply slippage tolerance."""

    logger = logging.getLogger(__name__)

    if slippage_percent < 0:
        raise ValueError("Slippage percent cannot be negative")

    try:
        amounts_out = router_contract.functions.getAmountsOut(amount_in, path).call()
    except ContractLogicError as exc:
        logger.error("Router getAmountsOut reverted for path %s: %s", path, exc)
        raise ValueError("Failed to quote swap output via getAmountsOut") from exc
    except ValueError as exc:
        logger.error("Router getAmountsOut raised ValueError for path %s", path, exc_info=exc)
        raise ValueError("Failed to quote swap output via getAmountsOut") from exc
    except Exception as exc:  # pylint: disable=broad-except
        logger.error(
            "Unexpected error invoking router getAmountsOut for path %s", path, exc_info=exc
        )
        raise ValueError("Failed to quote swap output via getAmountsOut") from exc

    if not amounts_out or len(amounts_out) != len(path):
        logger.error(
            "Router getAmountsOut returned unexpected result for path %s: %s",
            path,
            amounts_out,
        )
        raise ValueError("Invalid getAmountsOut response from router")

    output_amount = Decimal(amounts_out[-1])
    if output_amount <= 0:
        raise ValueError("Quoted output amount is non-positive")

    slippage_fraction = slippage_percent / Decimal(100)
    min_amount = output_amount * (Decimal(1) - slippage_fraction)
    if min_amount <= 0:
        raise ValueError("Slippage calculation resulted in non-positive minimum amount")

    min_amount_int = int(min_amount.to_integral_value(rounding=ROUND_DOWN))
    logger.debug(
        "getAmountsOut quoted %s for path %s; applying %s%% slippage => min_out=%s",
        output_amount,
        path,
        slippage_percent,
        min_amount_int,
